...
export class TurmaUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.turma_id = parseInt(this.route.snapshot.paramMap.get("id"));
    this.findById();
  }
  ...
  findById() {
    return this.service.findById(this.turma_id).subscribe(
      (response) => {
        this.turma = response;
      },
      (ex) => {
  ...
  update(): void {
    this.service.update(this.turma_id, this.turma).subscribe(
      (response) => {
        this.turma = response;
